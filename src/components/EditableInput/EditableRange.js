...
export default class EditableRange extends React.Component {
    ...
    onBlur(event) {
      let newMin = parseInt(document.getElementsByName("min")[0].value, 10);
      let newMax = parseInt(document.getElementsByName("max")[0].value, 10);
      ...
      return (
        <Fragment>
            <div className="container mw-100 p-0 text-center">
                <div className="row">
                  <div className="col-3">
                    <Input name="min" type="number" value={this.state.options.min} placeholder="min.." onBlur={(event) => this.onBlur(event)} onChange={(event) => this.handleMinChange(event)} />
                  </div>
                  <div className="col-6">
                    <Input type="range" disabled={"disabled"}/>
                  </div>
                  <div className="col-3">
                    <Input name="max" type="number" value={this.state.options.max} placeholder="max.." onBlur={(event) => this.onBlur(event)} onChange={(event) => this.handleMaxChange(event)} />
                  </div>
                </div>
            </div>
        </Fragment>
